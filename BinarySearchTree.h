...
template <typename T,typename Compare=std::less<T>> // default if argument isn't provided
...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static int height_impl(const Node *node) {
      if (!node) {
         ...
         // showing error, also how would it work if they are equal?
          //return 1 + maxer(height_impl(node->left), height_impl(node->right));
      return 1 + std::max(height_impl(node->left), height_impl(node->right));
      
    assert(false);
  }
    //helper max func for height
    static int maxer(const int x, const int y) {
        if (x > y) {
            return x;
        }
        return y;
        //what do we do if they are equal?
    }
  ...
  static Node *copy_nodes_impl(Node *node) {
      ...
      Node *now = new Node;
      *now = *node;
      now->left = copy_nodes_impl(node->left);
      now->right = copy_nodes_impl(node->right);
      return now;
      //should work, needs testing
  }
  ...
  static void destroy_nodes_impl(Node *node) {
      ...
      delete node; //unsure if this should go before or after
      //should work, needs testing
  }
  ...
  static Node * find_impl(Node *node, const T &query, Compare less) {
      if (empty_impl(node)) {
          ...
          //if equal return thise node ptr
          return node;
      }
      else if (node->left && !less(node->left->datum, query)
               && !less(query, node->left->datum)) {
          //if exists and equal return the left node
          return node->left;
      }
      else if (node->right && !less(node->right->datum, query)
               && !less(query, node->right->datum)) {
          //if exists and equal return the right node
          return node->right;
      }
      else if (node->left && less(query, node->left->datum)) {
          //if query is less than the left nodes datum, continue looking left
          return find_impl(node->left, query, less);
      }
      //if query is more than left nodes datum
      else if (node->right && less(node->right->datum, query)) {
          //if the right node's datum is less than query, continue looking
      ...
      else if (node->right && less(query, node->right->datum)
               && less(node->datum, query)) {
          return find_impl(node->right, query, less);
      }
      //added this above
      
      else {
          //should encompass everything left where:
          //query is less than current datum, but > than the left's datum
          //query is greater than the current datum, but > than the right's datum
          //add: less than now datum but left doesn't exist, and vice versa
          return nullptr;
      }
      //pretty sure itll return the pointer here that isn't null, or nullptr if all nul
      //needs testing
  }
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) {
      ...
      else {   //given no repeats, this should be if datum > item
      ...
      /*
      if (less(item, node->datum)) {   //iteam less than current node
          if (!node->left || !less(item, node->l
       eft->datum)) {   //item not less than the next one
              Node *now = new Node;
              now->datum = item;
              now->left = node->left;
              now->right = nullptr;
              ...
              node->left = insert_impl(node->left, item, less);
              return node;
              //hits errors here, deletes the original root node when it returns
              
          }
      }

      if (!less(item, node->datum)) { //item is greater than node
          if (!node->right || less(item, node->right->datum)) {
              Node *now = new Node;
              now->datum = item;
              now->left =  nullptr;
      ...
      //should only hit this line if there is an error
      return nullptr;
       */
  ...
  static Node * max_element_impl(Node *node) {
      ...
      if (!node->right) {
          // should it be datum?
          // i got rid of the *. It gave errors with * and -> datum so idk
          //needs testing
          return node;
      }
  ...
  // NOTE:    This function must be tree recursive.
    static bool check_sorting_invariant_impl(const Node *node, Compare less) {
        ...
        if (node == nullptr) {
        ...
        else if (node->left && !less(max_element_impl(node->left)->datum, node->datum)) {
            return false;
        }
            ...
            bool one = check_sorting_invariant_impl(node->left, less);
            bool two = check_sorting_invariant_impl(node->right, less);
            return (one && two);
        }
        ...
        if (node->right) { //if right exists
            ...
            if (!less(node -> datum, max_element_impl(node -> right)->datum ) &&
                !less(node->datum, node->right->datum)) { //added this
                //if the datum is NOT less than the max element in its right tree
        ...
        if (node->left) {  //if left exists
            //check this
            bool one = !less(min_element_impl(node->left)->datum, node->datum);
            bool two = !less(node->left->datum, node->datum);
            if (!less(min_element_impl(node->left)->datum, node->datum) &&
                !less(node->left->datum, node->datum)) { //added this
                //the min element in the left tree needs to be less than the current..
                //or return false
                return false;
            }
        }
        ...
        //recursive cal
         ...
         ls, one for left subtree and one for right sub
         tree, and not for when one doesnt exist because those are checks
        //check for current element, use less function and min/max
        //needs to be fixed
      //assert(false);
        assert(false);
         */
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
      ...
      //if equal, returns right node if there is one, else null
      ...
      if (less(node->datum, val) && node->right) {//datum less than val and right exist
          if (less(node->right->datum, val)) {  //right datum is less than val
              return min_greater_than_impl(node->r
       
       ight, val, less);  //THIS MIGHT BE WRONG UNSURE
          }
          else if (!less(node->right->datum, val) && !less(val, node->right->datum)) {
              //if they're equal
              //will prob cause an error cuz there is too many loops, keep for now
              
              if (node->right->right) {  //if right of right exists, return it
              ...
              return node->right;
              //return right if:
              //orig. datum is less than val
              //right datum is not less than val
              //right datum is not equal to val
          }
      }
      else if (less(node->datum, val) && !node->right) {
       // datum is less than val and no right node exists
          return nullptr;
      }
      ...
      // if val is less than given datum
      
      
      if (less(val, node->datum) && node->left) {
          if ( ) {  //datum is greater than val, left exists and val is > than left datum
