...
private:
  ...
  public:
      Key_compare Lizzie;
      bool operator () (const Pair_type one, const Pair_type two) {
          return Lizzie(one.first, two.first);
      }
...
public:
  ...
  //       using "Value_type()".
    Iterator find(const Key_type& k) const {
          ...
          Pair_type now;
          now.first = k;
          now.second = Value_type();

          Iterator search = mapTree.find(now);
          return search;
  ...
  // HINT: http://www.cplusplus.com/reference/map/map/operator[]/
    Value_type& operator[](const Key_type& k) {
        /*
         run a find function on key_Type& k
         if its find something, return it
         otherwise,
          insert new element with key and value at the end and returns it
                will be at the same poisition as given to us with find
        ...
        Iterator search = find(now.first);
        if (search == end()) {
            return mapTree.insert(now) -> second;
        }
        return find(now.first) -> second;
  ...
  //           the value true.
    std::pair<Iterator, bool> insert(const Pair_type &val) {
        pair<Iterator, bool> work;
        if (find(val.first) == end()) {  //originally mapTree.
            mapTree.insert(val);
            work.second = true;
        }
        else {
